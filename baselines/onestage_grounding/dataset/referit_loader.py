...
sys.path.append('.')
import utils
from utils import Corpus, Dictionary
...
class ReferDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', imsize=256,
        ...
        elif self.dataset == 'sk_vg':
            self.dataset_root = osp.join(self.data_root, 'sk_vg')
            self.im_dir = osp.join(self.dataset_root, 'images', "sk_vg")
            self.split_dir = osp.join(self.dataset_root, 'splits')
        else:  ## refcoco, etc.
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if augment_flip and random.random() > 0.5:
                ...
                phrase = phrase.replace('right', '*&^special^&*').replace('left', 'right').replace('*&^special^&*',
                                                                                                   'left')
