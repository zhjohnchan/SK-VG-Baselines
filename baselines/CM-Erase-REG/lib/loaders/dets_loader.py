...
class DetsLoader(Loader):
    ...
    #     ann_boxes = xywh_to_xyxy(np.vstack([self.Anns[det_id]['box'] for det_id in det_ids]))
    ...
    #     cxt_fc7, cxt_lfeats, cxt_ann_ids = self.fetch_cxt_feats(det_ids, opt)
    ...
    #             sent_ids += [sent_id]
    #             gd_ixs += [det_ids.index(ref['det_ids'])]
    ...
    def getTestBatch(self, split, opt):
        ...
        img_fc7 = torch.cat(batch_head_fc7, 0);
        img_fc7.detach_()
        ...
        data['Feats'] = {'pool5': pool5, 'fc7': fc7, 'lfeats': lfeats, 'dif_lfeats': dif_lfeats,
                         'cxt_fc7': cxt_fc7, 'cxt_lfeats': cxt_lfeats, 'img_pool5': img_fc7}
        data['labels'] = labels
        data['bounds'] = {'it_pos_now': self.iterators[split], 'it_max': max_index, 'wrapped': wrapped}
        return data
