...
class GtMRCNLoader(Loader):
    ...
    def __init__(self, data_json, data_h5):
        ...
        for image_id, image in self.Images.items():
            ...
            splits = set(self.Refs[ref_id]["split"] for ref_id in image['ref_ids'])
            for split in splits:
    ...
    def getBatch(self, split, opt):
        ...
        for image_id in batch_image_ids:
            ...
            for ref_id in ref_ids:
                ...
                # fetch feats of whole img
                head_pool5, _ = self.mrcn.img_to_spatial_fc7(head)
                for cnt in range(seq_per_ref):
                    batch_head_pool5 += [head_pool5]
        ...
        img_pool5 = torch.cat(batch_head_pool5, 0);
        img_pool5.detach_()
        ...
        data['Feats'] = {'fc7': pos_fc7, 'pool5': pos_pool5, 'lfeats': pos_lfeats, 'dif_lfeats': pos_dif_lfeats,
                         'cxt_fc7': pos_cxt_fc7, 'cxt_lfeats': pos_cxt_lfeats, 'img_pool5': img_pool5}
        ...
        data['neg_Feats'] = {'fc7': neg_fc7, 'pool5': neg_pool5, 'lfeats': neg_lfeats, 'dif_lfeats': neg_dif_lfeats,
                             'cxt_fc7': neg_cxt_fc7, 'cxt_lfeats': neg_cxt_lfeats, 'img_pool5': img_pool5}
    ...
    def fetch_cxt_feats_cat(self, ann_ids, opt):
        """
        Return
        - cxt_feats  : ndarray (#ann_ids, topK, fc7_dim)
        - cxt_lfeats : ndarray (#ann_ids, topK, 5)
        ...
        cxt_lfeats = np.zeros((len(ann_ids), topK, 5), dtype=np.float32)
        cxt_ann_ids = [[-1 for _ in range(topK)] for _ in range(len(ann_ids))]  # (#ann_ids, topK)
        cxt_category = np.zeros((len(ann_ids), topK), dtype=np.float32)
        for i, ref_ann_id in enumerate(ann_ids):
            # reference box
            rbox = self.Anns[ref_ann_id]['box']
            rcx, rcy, rw, rh = rbox[0] + rbox[2] / 2, rbox[1] + rbox[3] / 2, rbox[2], rbox[3]
            ...
            for j, cand_ann_id in enumerate(cand_ann_ids):
                ...
                cxt_feats[i, j, :] = self.feats['ann']['fc7'][cand_ann['h5_id'], :]
                cxt_ann_ids[i][j] = cand_ann_id
                cxt_category[i][j] = cand_ann['category_id']

        return cxt_feats, cxt_lfeats, cxt_ann_ids, cxt_category
    ...
    def getTestBatch(self, split, opt):
        ...
        head_pool5, _ = self.mrcn.img_to_spatial_fc7(
            head)  # actually here we use pool5 features. I was too lazy to revise the name. #_# ...
        batch_head_pool5 = []
        for cnt in range(len(ann_ids)):
            batch_head_pool5 += [head_pool5]
        img_pool5 = torch.cat(batch_head_pool5, 0)
        img_pool5.detach_()
        ...
        data['gd_ixs'] = gd_ixs
        data['Feats'] = {'pool5': pool5, 'fc7': fc7, 'lfeats': lfeats, 'dif_lfeats': dif_lfeats,
                         'cxt_fc7': cxt_fc7, 'cxt_lfeats': cxt_lfeats, 'img_pool5': img_pool5}
        data['labels'] = labels
        data['bounds'] = {'it_pos_now': self.iterators[split], 'it_max': max_index, 'wrapped': wrapped}
        return data
    ...
    def CatToVocab(self, category_id):
        # input: category
        # output: [vocab1, vocab2]
        cat_name = self.ix_to_cat[category_id]
        cat_label = []
        for wd in cat_name.split():
            cat_label.append(self.word_to_ix[wd])
        if len(cat_label) == 1:
            cat_label.append(self.word_to_ix['<PAD>'])
        return cat_label
