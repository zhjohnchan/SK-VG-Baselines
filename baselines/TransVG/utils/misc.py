...
def collate_fn(raw_batch):
    ...
    text_data = NestedTensor(word_id, word_mask)
    know_id = torch.tensor(raw_batch[4])
    know_mask = torch.tensor(raw_batch[5])
    know_data = NestedTensor(know_id, know_mask)
    bbox = torch.tensor(raw_batch[6])
    batch = [img_data, text_data, know_data, bbox]
    return tuple(batch)
