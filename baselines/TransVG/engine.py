...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable, 
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        img_data, text_data, know_data, target = batch

        # copy to GPU
        img_data = img_data.to(device)
        text_data = text_data.to(device)
        know_data = know_data.to(device)
        target = target.to(device)

        # model forward
        output = model(img_data, text_data, know_data)
...
def validate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        pred_boxes = model(img_data, text_data, know_data)
...
def evaluate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    iou, accu_num = eval_utils.trans_vg_eval_test(pred_boxes, gt_boxes)
    torch.save(iou, os.path.join(args.output_dir, "iou.pt"))
