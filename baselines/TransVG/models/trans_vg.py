...
from .vl_transformer import build_vl_transformer, build_vk_transformer
...
class TransVG(nn.Module):
    def __init__(self, args):
        ...
        if args.use_knowledge:
            self.know_proj = nn.Linear(self.textmodel.num_channels, hidden_dim)
            self.vk_transformer = build_vk_transformer(d_model=args.hidden_dim,
                                                       nhead=args.nheads,
    ...
    def forward(self, img_data, text_data, know_data):
        ...
        if self.args.use_knowledge:
            know_fea = self.textmodel(know_data)
            know_src, know_mask = know_fea.decompose()
            assert know_mask is not None
            know_src = self.know_proj(know_src)
            # permute BxLenxC to LenxBxC
            know_src = know_src.permute(1, 0, 2)
            know_mask = know_mask.flatten(1)

            # Knowledge-encoded Image Feats
            visu_src = self.vk_transformer(tgt=visu_src, memory=know_src, memory_key_padding_mask=know_mask.eq(0))
