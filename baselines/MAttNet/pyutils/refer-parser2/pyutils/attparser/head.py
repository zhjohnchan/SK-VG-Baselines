    ...
    while len(myqueue) > 0:
        ...
        if len(cur_label) >= label_len and cur_label[:label_len] == label:
            if cur_T[0] in not_allowed:  # in case parser take 'first', 'second' as noun
                continue
            if cur_T[0][-3:] == 'ing' and cur_T[
                0] not in ing_allowed:  # in case parser take both 'man' and 'standing' as Noun for 'man standing under the tree'
                continue
            return cur_T
        else:
    ...
    while len(myqueue) > 0:
        ...
        if len(cur_label) >= label_len and cur_label[:label_len] == label:
            if cur_T[0] in not_allowed:
                ...
                continue
            if cur_T[0][-3:] == 'ing' and cur_T[
                0] not in ing_allowed:  # in case parser take 'standing' as Noun for 'man standing under the tree'
                continue
            return cur_T
        else:
            if not isinstance(cur_T[0], str):  # if not the leaf node, i.e., 'dog', 'tree'
